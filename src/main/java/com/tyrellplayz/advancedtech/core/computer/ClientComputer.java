...
import com.tyrellplayz.advancedtech.api.content.dialog.Dialog;
...
public class ClientComputer extends GuiScreen implements ApplicationSystem, IFileSystem {
    ...
    public Application getApplication(ResourceLocation id) {
        if (!AdvancedTech.getApplicationManager().isApplicationLoaded(id)) {
            return null;
        } else {
            ApplicationManifest applicationManifest = AdvancedTech.getApplicationManager().getApplication(id).getManifest();
    ...
    public Collection<ApplicationManifest> getInstalledApplications() {
        return AdvancedTech.getApplicationManager().getApplications().stream().map(ApplicationManager.ApplicationData::getManifest).sorted().collect(Collectors.toList());
    }
    ...
    public Application openApplication(ResourceLocation id) {
        if (this.isApplicationOpen(id)) {
            ...
            Application application = AdvancedTech.getApplicationManager().createApplication(id);
            if (application == null) {
                return null;
            } else {
    ...
    public void updateData() {
        this.systemData.put("FileSystem", this.fileSystem.read());
